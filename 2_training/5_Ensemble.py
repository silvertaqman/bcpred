...
results = pd.DataFrame(p.cv_results_)
Features = list(results.columns)
params = pd.DataFrame(results,columns=Features)
params.to_csv("./ensemble_metrics/gsdtc.csv", index=False)
...
stack = StackingClassifier(estimators = estimators, final_estimator = lr).fit(X, y)

joblib.dump(stack, "./ensemble_models/stacking.pkl")

# Stacking bagged & boosted ensembles
...
estimators_ada = [("svm", svmrbf),("lr",lr),("dtc",adadtc)]

stack_ada = StackingClassifier(estimators = estimators_ada, final_estimator = lr).fit(X, y)

#joblib.dump(stack_bag, "./ensemble_models/stack_bag.pkl")

joblib.dump(stack_ada, "./ensemble_models/stack_ada.pkl")
